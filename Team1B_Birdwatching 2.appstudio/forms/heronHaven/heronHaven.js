...
btnSeeInfo1.onclick=function(){
        ...
        query = "SELECT description, picture_link FROM picture p INNER JOIN bird b ON b.bird_id = p.bird_id INNER JOIN bird_species_to_park bp ON b.bird_id = bp.bird_id INNER JOIN nature_parks n ON n.park_id = bp.park_id WHERE park_name = 'Heron Haven Nature Center' AND name = " + '"' + selectBird1 + '"'
        req = Ajax("https://ormond.creighton.edu/courses/375/ajax-connection.php", "POST", "host=ormond.creighton.edu&user=kxa57381&pass=" + pw + "&database=375groupb1&query=" + query)
        console.log(req)
    if (req.status == 200) { //transit worked.
